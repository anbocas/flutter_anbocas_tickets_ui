...
class AnbocasPaginatedOrder {
  PaginatedOrderData? data;
  Status? status;

  AnbocasPaginatedOrder({this.data, this.status});

  AnbocasPaginatedOrder.fromJson(Map<String, dynamic> json) {
    if (json["data"] is Map) {
      data = json["data"] == null
          ? null
          : PaginatedOrderData.fromJson(json["data"]);
    }
  ...
  static List<AnbocasPaginatedOrder> fromList(List<Map<String, dynamic>> list) {
    return list.map(AnbocasPaginatedOrder.fromJson).toList();
  }
